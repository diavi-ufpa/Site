...
# --- 1. Caminhos de Arquivos e Planilhas por Ano/Período ---
...
bases_avalia <- list(
  ...
  "2024-4" = list(
    dataSource    = "databases/AVALIA-2024-4.xlsx",
    sheetDiscente = "DISCENTE",
    sheetDocente  = "DOCENTES"
  ),

  "2025-2" = list(
  dataSource    = "databases/AVALIA_2025-2.xlsx",
  sheetDiscente = "DISCENTE",
     sheetDocente  = "DOCENTE"
   ),

  "2025-4" = list(
  dataSource    = "databases/AVALIA-2025-4.xlsx",
  sheetDiscente = "DISCENTES",
     sheetDocente  = "DOCENTES"
   ))
  
get_base_config <- function(ano) {
  ano <- trimws(as.character(ano))

  if (is.null(ano) || !nzchar(ano)) {
    stop("Parâmetro 'ano' é obrigatório.")
  }

  cfg <- bases_avalia[[ano]]

  if (is.null(cfg)) {
    stop(sprintf("Ano/período inválido: %s", ano))
  }
...
# --- 5. Mapeamento de Subdimensões (DOCENTE) ---
# Usadas pelos endpoints /docente/autoavaliacao/subdimensoes/*

subdim_map_doc <- list(
  "Atitude Profissional" = colsAtProfissionalDoc,
  "Gestão Didática"      = colsGestaoDidaticaDoc,
  "Processo Avaliativo"  = colsProcAvaliativoDoc
)


# --- 6. Mapeamento de Subdimensões (DISCENTE) ---
# Útil para endpoints /discente/acaodocente/subdimensoes/*

subdim_map_disc <- list(
  "Atitude Profissional" = colsAtProfissional,
  "Gestão Didática"      = colsGestaoDidatica,
  "Processo Avaliativo"  = colsProcAvaliativo
)